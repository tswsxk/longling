...
class BasePredictor(object):
    def __init__(self, prefix, epoch=0, batch_size=128,
                 load_optimizer_states=False, **kwargs):
        ...
        self.mod = mx.module.Module.load(prefix, epoch, load_optimizer_states,
                                         **kwargs)
        self.mod.bind(data_shapes=[('data', (batch_size,) + self.data_shapes)],
                      force_rebind=True)
        self.batch_size = batch_size

    def predict(self, datas, label_tag=False):
        batch_size = min(len(datas), self.batch_size)
        datas = mx.io.NDArrayIter(data=np.asarray(datas), batch_size=batch_size,
                                  data_name="data")
        self.mod.bind(data_shapes=[('data', (batch_size,) + self.data_shapes)],
                      force_rebind=True)
...
class RNNPredictor(object):
    def __init__(self, prefix, cells, sym, epoch=0, batch_size=128,
                 buckets=[10, 20, 30], **kwargs):
        if buckets:
            _, arg_params, aux_params = mx.rnn.load_rnn_checkpoint(cells,
                                                                   prefix,
                                                                   epoch)
    ...
    def predict(self, datas, label_tag=False):
        result = []
        datas, _, origin_idx = SimpleBucketIter.bucket_sort(datas,
                                                            sorted_key=lambda
                                                                x: len(x))
        ...
        for i, data in enumerate(data_buff):
            if not data:
                continue
            pad_num = SimpleBucketIter.padding(data, self.batch_size,
                                               [self.data_pad] * self.buckets[
                                                   i])
            datas_iter = SimpleBucketIter(self.batch_size, data,
                                          buckets=self.buckets,
                                          for_predicting=True)
