...
def show(key, max_key=True, exp_id=None, res_dir="./", nni_dir=path_append(os.environ.get("HOME", "./"), "nni/experiments"),
         only_final=False,
    ...
    for trial in [row[0] for row in cursor]:
        ...
        if max_key:
            ...
            value, appendix = get_min(trial_res, key, with_keys=with_keys, with_all=with_all)
...
def prepare_hyper_search(cfg_kwargs: dict, cfg_cls, reporthook=None, final_reporthook=None,
                         primary_key=None, max_key=True, reporter_cls=None, with_keys: (list, str, None) = None, dump=False):
    try:
        import nni
        ...
        class Reporter(BaseReporter):
            ...
            def final(self):
                ...
                final_result = best_fn(self.datas, primary_key, with_keys=";".join(with_keys) if with_keys else None)
                feed_dict = {
